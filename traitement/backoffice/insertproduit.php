...
if (isset($_SESSION['user']) && $_SESSION['user']['ID_role']==2) {
    ...
    foreach ($donnee as $data) {
        ...
        <td><a href='index.php?page=updateproduit&update=$data[ID_produit]'>Modifier</a></td> 
        </tr>";
    }
    echo'</table>';
}
...
<section class= "renseigner">
    <form action="" method="post" enctype="multipart/form-data">
        <select name="categorie" id="selectcat" required>
            ...
            //on appelle la fonction qui selectionne toutes les categories afin de les afficher dans un menu deroulant(balise selectHTML)
...
if (isset($_SESSION['user'])&& $_SESSION['user']['ID_role']==2 ) {
        if(isset($_POST['produit'])){
            $produit=$_POST['produit'];
                ...
                //on compare les produits que nous avons en DB avec celui que l'utilisateur encode 
                foreach ($donnee2 as $data){
                    // si il y a un produit exactement identique on envoie un message à l'utilisateur 
                    if ($data['ID_categorie']==$categorie && $data['nom_produit']==$produit && $data['prix_produit']==$prix) {
                        $exist = true;
